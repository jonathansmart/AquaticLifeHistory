...
vignette: >
  %\VignetteIndexEntry{Maturity estimation}
...
```{r bootstrapped, warning=FALSE,  fig.height = 6, fig.width =7}
...
```{r with points bootstrapped, warning=FALSE,  fig.height = 6, fig.width =7}
# selecting return = "plot" returns a ggplot object rather than the parameters.
...
```{r quasi age plot, warning=FALSE,  fig.height = 6, fig.width =7}
# selecting return = "plot" returns a ggplot object rather than the parameters.
...
```{r length parameters, message=FALSE, warning=FALSE}
...
```{r bootstrapped length, warning=FALSE,  fig.height = 6, fig.width =7}
# selecting return = "plot" returns a ggplot object rather than the parameters.
...
```{r length unit changed, warning=FALSE,  fig.height = 6, fig.width =7}
...
An additional argument is needed to use a quasibinomial model. Previously, we binned age by year class as there is no other sensible way to go about this. However, for length the bin width needs to be specified and this is very important. This is done using the `bin.width` argument.


```{r bin width, warning=FALSE,  fig.height = 6, fig.width =8}
# selecting return = "plot" returns a ggplot object rather than the parameters.
Estimate_Len_Maturity(maturity_data,
                      return = "plot",
                      n.bootstraps = 1000,
...
```{r bin width comparison, warning=FALSE}
# Binomial model
Estimate_Len_Maturity(maturity_data)
